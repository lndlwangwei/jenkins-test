def build(envProps) {
    ...
    stage('build') {
        ...
        sh 'mvn clean'
        sh "mvn install -Dmaven.test.skip=true -P ${envProps.env}"

        dir('console-website') {
            sh 'npm install'
            sh "ng build --configuration=${envProps.env}; cd dist/; zip -r ../rbm_${envProps.env}_client.zip ./"
        }
        ...
        archiveArtifacts "console-website/rbm_${envProps.envF}_client.zip"
    }
}
