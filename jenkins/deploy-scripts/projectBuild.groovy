build(envVar) {
    ...
    stage('build') {
        ...
        sh 'mvn clean'
        sh "mvn install -Dmaven.test.skip=true -P ${envVar}"

        dir('console-website') {
            sh 'npm install'
            sh "ng build --configuration=${envVar}; cd dist/; zip -r ../rbm_${envVar}_client.zip ./"
        }
        ...
        archiveArtifacts "console-website/rbm_${envVar}_client.zip"
    }
}
